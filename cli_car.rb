	...
	def menu_car
		...
		until option == 'exit' do
			...
			when '1'
				system 'clear'
				puts '-- Create Car --'
				result = create_vehicle(type: 'car')
				...
				print 'Success create new car.'
				gets.chomp
			when '2'
				show_vehicle(vehicles: Motor.where(type: 'car'))
				gets.chomp
			when '3'
				vehicle_to_choose = ''
				vehicles = Motor.where(type: 'car')
				...
				until ((is_number?(vehicle_to_choose) && vehicle_to_choose.to_i <= vehicle_count && vehicle_to_choose.to_i > 0) || vehicle_to_choose == 'exit')
					...
					print "Choose car by number [type 'exit' to back] : "
		...
		until option_operate_vehicle == 'exit' do
			...
			when '3'
				...
				if is_number?(run_time)
					...
					print "Car run for #{result[0].to_digits} s and reached #{result[1].to_digits} M."
					gets.chomp
				end
			when '4'
